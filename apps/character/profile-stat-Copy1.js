...
export async function profileStat (e) {
  ...
  lodash.forEach(talentData, (avatar) => {
    ...
    if (profiles[id]) {
      ...
      let temp = Data.getData(mark, 'mark,markClass,names')
      global.alter = 1
      mark = profiles[id].getArtisMark(false)
      let temp2 = Data.getData(mark, 'mark,markClass,names')
      global.alter = 2
      mark = profiles[id].getArtisMark(false)
      let temp3 = Data.getData(mark, 'mark,markClass,names')
      global.alter = 3
      mark = profiles[id].getArtisMark(false)
      let temp4 = Data.getData(mark, 'mark,markClass,names')
      if (temp3['mark']*1 >= temp2['mark']*1 && temp3['mark']*1 >= temp['mark']*1&& temp3['mark']*1 >= temp4['mark']*1){
          avatar.artisMark = temp3
          avatar.artisMark['mark'] += '核爆'
      }else if (temp2['mark']*1 >= temp3['mark']*1 && temp2['mark']*1 >= temp['mark']*1&& temp2['mark']*1 >= temp4['mark']*1){
          avatar.artisMark = temp2
          avatar.artisMark['mark'] += '纯色'
      }else if (temp4['mark']*1 >= temp3['mark']*1 && temp4['mark']*1 >= temp['mark']*1&& temp4['mark']*1 >= temp2['mark']*1){
          avatar.artisMark = temp4
          avatar.artisMark['mark'] += '默认'
      }else{
          avatar.artisMark = temp
      }
    }
  })

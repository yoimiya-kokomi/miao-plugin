...
export async function renderProfile (e, char, mode = 'profile', params = {}) {
  ...
  let ren_name = char.name
  let { artis, mark: totalMark, markClass: totalMarkClass, usefulMark } = profile.getArtisMark()
  if (global.alter===3) {
     ren_name = ren_name  +'-六连'
     totalMark = (totalMark*1)+Math.max(Math.min((profile.attr.cpct*1-75)/(75)*100,20),-20)
    }
  if (global.alter===1) {
     ren_name = ren_name  +'-纯色'
    }
  if (global.alter===2) {
     ren_name = ren_name  +'-核爆'
    }
